...
nvsh registers eleven harness adapters (nine harnesses plus the in-process
`needle` and `lfm` tier adapters) in `nvsh/agent/registry.py`'s
...
# DeepEval release gate (issue 64): its own dependency group, outside tests/
uv sync --group evals
