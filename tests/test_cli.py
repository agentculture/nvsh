"""Smoke tests for the nvsh CLI entry point and its verbs."""
...
from nvsh.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: nvsh" in capsys.readouterr().out
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "nvsh whoami" in payload["markdown"]
