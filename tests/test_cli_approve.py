...
def test_approve_add_session_not_in_new_process_list(capsys):
    rc = main(["approve", "add", "kubectl get *", "--session"])
    assert rc == 0
    capsys.readouterr()
    # A fresh `nvsh approve list` invocation starts a new in-memory session,
    # so the session-scoped pattern must not appear (nor persist to disk).
    rc = main(["approve", "list", "--json"])
    assert rc == 0
    payload = json.loads(capsys.readouterr().out)
