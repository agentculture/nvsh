...
- obligation: `o1` (criterion 1) [docs/architecture.md] opens with the before-state and cites README.md:10-14 and 0.9.1's PTY wording; records hook-over-wrap and the parked login-shell mode
- obligation: `o2` (criterion 2) [the five prompt files + scripts/harness-smoke.py --stage config] every prompt file describes the hook architecture; 'Login-shell constraints' is replaced by 'Hook constraints'; the config stage passes
- obligation: `o3` (criterion 3) [README.md + markdownlint-cli2] first paragraph names the machines, bash, Ghostty and ssh; a 'What leaves the machine' section exists; lint passes
...
- obligation: `o4` (criterion 1) [tests/`test_triggers.py` table] one row per CLAUDE.md trigger rule plus the remote pass-through set; every non-error row yields skip with a reason
- obligation: `o5` (criterion 2) [nvsh/triggers.py decide() on the listed cases] 130, 141, grep/diff 1, false, test, in-script -> skip; 'ls /nope' exit 2 -> ask; second auto call inside the window refused and recorded
- obligation: `o6` (criterion 3) [python -X importtime nvsh.triggers + decide() signature] stdlib-only imports; Decision carries action, reason, `rule_id`
...
- obligation: `o7` (criterion 1) [tests/`test_redact.py` corpus file] every listed secret class is replaced by a typed marker and no secret substring survives
- obligation: `o8` (criterion 2) [redact(bytes) property test] invalid UTF-8 and escape sequences never raise; redact(redact(x)) == redact(x)
...
- obligation: `o9` (criterion 1) [nvsh/platform/detect.py against the spark, thor and orin fixture trees] kind and every (value, source) pair match the recorded machines, including absences
- obligation: `o10` (criterion 2) [unified-memory mapping + presence table] nvidia-smi \[N/A\] maps to unified mode with MemAvailable; nvpmodel, cuDNN, TensorRT, tmux, pi, spark are each reported present or absent
- obligation: `o11` (criterion 3) [docs/platforms.md] every value lists source, file-or-subprocess, and the verifying command for all three machines
...
- obligation: `o12` (criterion 1) [nvsh/config.py load()] tomllib parse of the XDG config; documented defaults when missing; no API key field is ever read or written
- obligation: `o13` (criterion 2) [nvsh/approvals.py (tests/`test_approvals.py`)] approved.toml 0600; fnmatch on full command; add() refuses sudo, rm and bare \*; default read-only list present; session list in memory
- obligation: `o14` (criterion 3) [scripts/scan-secrets.py over docs/ and nvsh/] passes; examples use <http://localhost> placeholders only
...
- obligation: `o15` (criterion 1) [nvsh/agent/base.py types] NvshAgent, AgentRequest, AgentContext and the six event kinds exist; grep finds no pi import outside nvsh/agent/pi.py
- obligation: `o16` (criterion 2) [tests/`test_agent_conformance.py` fixture over every registered adapter] streaming order, cancel mid-stream, error propagation, capability report and teardown pass for FakeAgent
- obligation: `o17` (criterion 3) [tests/`test_approval_loop.py` + audit.jsonl] 'sudo rm -rf /x' is rendered and not executed until approve() is True; retry and verify follow; proposal, decision, outcome logged; `READLINE_LINE` never written
...
- obligation: `o18` (criterion 1) [tests/`test_readline_bash.py` Enter routing in emacs and vi modes] /doctor and '/ask why' route with the original line in history; /notacmd, 'ls -d /tmp' and a quoted two-line echo pass through; Ctrl+G prints and restores
- obligation: `o19` (criterion 2) [tests/`test_readline_bash.py` completion cases] '/'+Tab merged palette, '/do'+Tab -> /doctor, '/tm'+Tab -> /tmp/, '/doctor '+Tab arguments from 'nvsh complete --json', 'ec'+Tab -> echo; complete -D untouched
- obligation: `o20` (criterion 3) [erroring hook function under script(1)] Enter still accepts the line
...
- obligation: `o21` (criterion 1) [nvsh/capture.py `open_session_log` (tests/`test_capture.py`)] 0600 log under the runtime dir; refuses when already under script/tmux or `NVSH_WRAPPED`; removed at exit; script(1) death returns 'no capture'
- obligation: `o22` (criterion 2) [nvsh/capture.py `last_slice` with the scratchpad fixture] exact C..D bytes ('hello-out' + ls error), 64 KB cap with head+tail marker, escapes stripped, UTF-8 safe, log path absent
- obligation: `o23` (criterion 3) [tmux pipe-pane source (\$TMUX set)] the same log is fed by pipe-pane; only the slice reaches the redactor
...
- obligation: `o24` (criterion 1) [PiAgent argv assertion (tests/`test_pi_agent.py`)] argv contains the hygiene flags, the nvsh session dir and the approval extension; reader splits on LF only; rpc events map to NvshAgent events
- obligation: `o25` (criterion 2) [PiAgent.cancel() and process death] abort sent and returned within 1 s; killed pi yields an error event; no file under ~/.pi/agent/sessions (HOME in tmp)
- obligation: `o26` (criterion 3) [tests/fakes/pi rpc script] the t6 conformance suite passes against PiAgent with no node installed
...
- obligation: `o27` (criterion 1) ['nvsh agent list --json' / 'nvsh agent use'] five harnesses with installed status; use writes config; with none installed the panel text offers install pi or choose another
- obligation: `o28` (criterion 2) [OpenAICompatAgent against a local fake HTTP server] stdlib urllib streaming, 5 s connect timeout, bearer from an env var name; conformance passes
- obligation: `o29` (criterion 3) [claude/codex/qwen adapters via fake scripts on PATH] each maps its headless mode to NvshAgent events with a capability declaration and passes conformance; setup on a node-less host selects openai-compat and says why
...
- obligation: `o30` (criterion 1) [approval.ts `tool_call` handler] bash calls go through 'nvsh approve check --json'; user/session allow; ask raises `extension_ui_request` with once/session/user; others blocked
- obligation: `o31` (criterion 2) [Node-free .ts parse test + live test skipped without pi] nvidia-smi runs; 'apt install foo' asks then runs for the session then asks again in a new daemon; 'sudo rm -rf /x' always asks; all three audited
- obligation: `o32` (criterion 3) [wheel contents + PiAgent path] approval.ts ships in the wheel and PiAgent references it by absolute path
...
- obligation: `o33` (criterion 1) [nvsh/daemon.py lifecycle (pgrep test)] no process at shell start; one daemon and one agent process after the first failure; none after the last shell exits or idle timeout
- obligation: `o34` (criterion 2) [tests/`test_daemon.py` two-shell scenario] A and B conversations stay separate; one pi process; sleep/resume via `switch_session`; sessions.max=2 allows a second process
- obligation: `o35` (criterion 3) [daemon-missing and pi-absent paths] one-shot adapter fallback; 'pi unavailable' reported and the configured fallback adapter used
...
- obligation: `o36` (criterion 1) [nvsh/client.py context assembly (tests/`test_context.py`)] AgentContext holds hook args, redacted slice and platform block with sources; 'nvsh context --show' equals the sent bytes; localhost reachable with sockets disabled
- obligation: `o37` (criterion 2) [nvsh/panel.py (tests/`test_panel.py`)] SGR only, no tput; readable under TERM=dumb, `NO_COLOR`=1, non-tty and missing terminfo; Enter/e/d/Esc keys; proposal shown verbatim, never pre-typed
- obligation: `o38` (criterion 3) [SIGINT path + last-failure.json] Ctrl+C aborts and restores within 1 s; /fix finds the last failure; retry only after Enter; verify reports the new status
...
- obligation: `o39` (criterion 1) [nvsh/slash.py registry + 'nvsh complete --json'] items carry value and description; platform-aware filtering hides Jetson-only commands on the Spark; '/doctor' argument items; no command list in bash
- obligation: `o40` (criterion 2) ['nvsh slash', /undo, /approve + teken cli doctor --strict] slash routes with kind slash; /undo drops the last turn and never calls the executor; /approve edits the lists; every verb has a catalog entry and the rubric passes
...
- obligation: `o48` (criterion 1) [tests/`test_hook_bash.py` under script(1)] zero nvsh forks across 1000 successes, under 5 ms median added; PIPESTATUS '1 0' and '0 1 0' recorded; pipefail yields 1
- obligation: `o49` (criterion 2) [Ghostty composition + `NVSH_DISABLE`] nvsh hook first and `__ghostty_hook` once; no nvsh DEBUG trap; `NVSH_DISABLE`=1 makes the file a no-op
- obligation: `o50` (criterion 3) [bash-side pre-filter] the Python entrypoint is called only when exit not in 0/130/141, the line is non-empty and the shell is not inside a sourced script, with exit, PIPESTATUS, line, cwd and log path
...
- obligation: `o51` (criterion 1) [nvsh/rcfile.py (tests/`test_rcfile.py`)] one marked block right after the interactive guard (fallback top of file); timestamped backup; idempotent; heavy-rc startup within 10% of baseline
- obligation: `o52` (criterion 2) [version stamp, uninstall, on/off (tests/`test_setup.py`)] refresh notice once per session; uninstall restores the rc byte-identical to the backup and removes files, sockets, logs, daemons; off/on unbind and rebind
- obligation: `o53` (criterion 3) [uv build + clean venv install] wheel contains nvsh/shell/\*.bash and nvsh/agent/`pi_ext`/\*.ts; 'nvsh setup' works; dependencies empty
...
- obligation: `o41` (criterion 1) ['nvsh doctor --json' new checks] `platform_detected`, `agent_configured`, `agent_reachable` (three remediations), `hook_sourced`, `hook_first_in_prompt_command`, `bindings_present`, `capture_active`, `daemon_status`, `terminfo_present` in the rubric shape
- obligation: `o42` (criterion 2) [hooked test shell with the hook moved last; host without xterm-ghostty terminfo; wheel install] ordering check fails with the fix command; terminfo remediation is the tic line; every new check runs without culture.yaml
...
- obligation: `o43` (criterion 1) [CI lint/test/rubric commands as .github/workflows/tests.yml runs them] all pass; coverage at or above 60%; importtime lists only stdlib
- obligation: `o44` (criterion 2) [git diff on the pinned invariants + PATH-stripping test] empty diff; pinned tests pass; without pi, tmux, fzf and spark every verb still works with diagnostics
...
- obligation: `o45` (criterion 1) [docs/verification.md dated checklist] each line for spark (Ghostty), thor and orin (ssh, with and without tmux) marked pass/fail with observed output
- obligation: `o46` (criterion 2) [tests/`test_timing.py` + docs/demos/] under 5 ms on success and under 2 s to first text with the fake agent, numbers recorded next to the targets; two recordings stored
- obligation: `o47` (criterion 3) [live Spark session check] `PROMPT_COMMAND` shows both hooks once with nvsh first; Ghostty jump-to-prompt works across a panel; no flicker; orin panel via openai-compat
