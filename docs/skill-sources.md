...
nvsh vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. nvsh
...
#   - consumer-identifying prose: `guildmaster` → `nvsh` (NOT
...
If a re-sync would lose a nvsh adaptation, lift the change
...
#  `which nvsh vendors from guildmaster`).
