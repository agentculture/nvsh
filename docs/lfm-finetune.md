...
**escalate** to the full agent. Stock LFM2.5-350M almost never proposes or
escalates. This page is the recipe for tuning it on nvsh's own data,
measuring whether the tuning worked, and publishing the result.

**Status: run end to end on a DGX Spark, 2026-09-22 and 23 (issue 39).**
Every command below was executed unless it is marked *(unverified)*. The
dated [run log](#run-log-issue-39) at the end records what happened at each
step, including the runs that failed.
...
A fine-tune succeeds when it measurably improves on the target, not merely
when training runs. It is judged against stock re-measured on the same
**test** side of the seeded split, a side no run was trained on or chosen
with:

- **Right proposals** of at least 70%, and **escalations** of at least 60%;
- **explain asks** answered in words at least as often as stock;
- **0 wrong mutating proposals**: no mutating operation other than the
  expected one, and no expected mutating operation with wrong arguments;
- a **warm median under 500 ms** through the real Tier 2 launcher.

The last rule is the hard one. A model that proposes the wrong change, even
once, is not adopted, however good its other numbers are.

## Where it stands

The final test run (`docs/benchmarks/2026-09-23-lfm-final-r8.md`), stock and
the chosen run r8 back to back:

| | Stock | r8 | Floor |
|---|---|---|---|
| Right proposals | 0 of 32 | 28 of 32 (88%) | met |
| Escalations | 0 of 15 | 13 of 15 (87%) | met |
| Explain asks explained | 15 of 17 | 17 of 17 | met |
...
On the DGX Spark (GB10, aarch64, CUDA 13.0), a virtual environment outside
the repository:
...
This gives unsloth 2026.9.9, transformers 5.5.0, trl 0.24.0, peft 0.21.0 and
torch 2.12.1+cu130. Installing unsloth replaces the torch from the first
step; CUDA still works.

### 2. Split the data

```bash
python scripts/lfm-finetune/split.py --out-dir work/splits --seed 39
```

This makes a seeded 70/15/15 train/validation/test split of
`nvsh/tiers/corpus/dev.json`, stratified by answer (operation, escalate,
explain). The split script refuses the held-out file and duplicate ids, keeps
variations on the same side as their source, and fails if a kind cannot
reach every side. **Iterate on validation only.** The test side is measured
once per final run, with `--final`, and nothing is chosen from it.

### 3. Measure stock first

```bash
python scripts/lfm-finetune/measure.py --split work/splits/val.json \
  --model LiquidAI/LFM2.5-350M --revision 9e6c6ccf47cd318696e137d381a7ded8fe4df09f \
  --label stock-val --config measure-config.toml --out stock-val.md
```

`measure.py` runs `nvsh tiers bench` for Tier 2 through the real launcher. It
records the model revision (checked against the cache's `refs/main`), the
container settings, and what else was running. `measure-config.toml` is a
measurement-only nvsh config: `[tiers] enabled = true` plus a `[tiers.lfm]`
table with `engine = "vllm"`, the pinned image, `hf_cache_dir`,
`tool_call_parser = "lfm2"` and `hf_offline = true` (see
[`tier2.md`](tier2.md)).

### 4. Build the training set from the train side

```bash
python scripts/lfm-finetune/build_dataset.py --split work/splits/train.json --out nvsh-train.jsonl
```

Each line is `{"messages": [...], "tools": [...]}`. The system brief, the
tools and the user message come from the same functions Tier 2 uses at run
time, so the model is trained on exactly what it will see. Three answers are
taught: `propose(operation, arguments)`, `escalate(reason)` and
`explain(text)`, the last one for the corpus's `{"explain": true, "answer":
...}` entries. The builder accepts only a train-side file.

Two things about the LFM2.5 chat template matter here:

- tool-call arguments must be a JSON **object**. The template refuses a
  JSON-encoded string;
- the template renders arguments in the order they are stored. `propose`
  must be written with `operation` before `arguments`. Sorted keys put
  `arguments` first, and the resulting run (r4) proposed nothing right.

### 5. Add data (optional, and what worked)

Around 300 train entries got the model to 20 of 32 on validation. Two
additions took it to 28:

- **Variations** (`augment.py`): up to 30 rewrites of each train entry. One
  local model writes each rewrite and a second copyedits it. Two reviewer
  models must both accept it, and deterministic checks reject any rewrite
  that names an operation identifier or copies the answer's wording. The
  generator and copyeditor never see the expected answer. Each variation
  keeps its source's side and answer.
- **A small authored supplement** (`train-supplement.json`) for gaps
  validation exposed. The corpus had no stop or shut-down request, and the
  model turned "Stop the inference container" into a restart. The
  supplement adds stop and disable requests that escalate, next to restart
  requests that propose a restart. It is merged into the train side only.

`merge_variations.py` folds both into the train split, and `build_dataset.py`
builds from the result.

### 6. Train

```bash
lfm-train/.venv/bin/python scripts/lfm-finetune/train.py --train nvsh-train.jsonl \
  --out runs/r8 --base LiquidAI/LFM2.5-350M --revision 9e6c6ccf47cd318696e137d381a7ded8fe4df09f \
  --epochs 20 --lr 5e-4 --rank 32 --alpha 64 --batch 8 --seed 7
```

`train.py` trains a LoRA adapter with the loss on the assistant turn only.
The mask comes from `apply_chat_template(..., return_assistant_tokens_mask=True)`
and covers exactly the tool call and its end token. It then merges the
adapter with plain `transformers` + `peft` into `runs/<name>/merged`. r8 took
about 35 minutes for 1040 examples on the GB10.

### 7. Stage and check

```bash
python scripts/lfm-finetune/stage_cache.py --merged runs/r8/merged \
  --repo jetson-ai-lab/lfm2.5-350m-nvsh-triage --cache <hf_cache_dir> \
  --base-snapshot <hf_cache_dir>/hub/models--LiquidAI--LFM2.5-350M/snapshots/9e6c6ccf47cd318696e137d381a7ded8fe4df09f
```

This puts the checkpoint into the Hugging Face cache under a repository id,
so the real launcher can serve it with `hf_offline = true` and no token. It
refuses a chat template that differs from the base's. The revision is a
hash of the files, so a report names exactly the weights it measured.

### 8. Measure on validation, then once on test

`measure.py --split work/splits/val.json --details r8-val.jsonl` gives the
same table as stock, plus one line per entry, for iterating. When a run is
chosen, measure stock and that run back to back on the test side with
`--final` (no per-entry details), and read the result against the floors
above.

### 9. Publish, privately

```bash
python scripts/lfm-finetune/release_bundle.py --merged runs/r8/merged \
  --base-snapshot <base snapshot> --repo jetson-ai-lab/lfm2.5-350m-nvsh-triage \
  --run r8 --results r8-val.md --data-summary "<what it was trained on>" --out bundle-r8
grant run --inject HF_TOKEN=HF_TOKEN -- hf repos create jetson-ai-lab/lfm2.5-350m-nvsh-triage --private
grant run --inject HF_TOKEN=HF_TOKEN -- hf upload jetson-ai-lab/lfm2.5-350m-nvsh-triage bundle-r8 .
```

The bundle carries the base model's LICENSE byte for byte, a NOTICE that
states the modification, and a model card with the licence, the per-user
commercial threshold, the results and the teacher models. Names agreed
...
```toml
[tiers.lfm]
model = "jetson-ai-lab/lfm2.5-350m-nvsh-triage"
engine = "vllm"
tool_call_parser = "lfm2"
hf_offline = true
```

Fetch the model into `hf_cache_dir` on the host first (`grant run --inject
HF_TOKEN=HF_TOKEN -- hf download ...`). The container gets no token, and no
nvsh runtime change is needed.

## Pitfalls hit, and the fix for each

- **Render the chat template before building a `datasets.Dataset`.**
  `Dataset.from_list` merges every tool's schema, so each tool call gains
  `null` keys for every other tool's parameters. `train.py` tokenizes each
  row first.
- **unsloth's `save_pretrained_merged` fails** on the read-only cache files.
  Merge with `PeftModel.from_pretrained(base, adapter).merge_and_unload()`
  instead; `train.py --merge-only <adapter>` redoes just that step.
- **Argument order** (step 4): write `operation` before `arguments`.
- **Too many epochs overfit a small set**: 8 epochs on the first 301
  examples gave 12 of 32; 20 epochs gave 20.
- **Reviewers and generators need reading, not trusting.** Five prompt
  faults were found only by reading accepted and rejected samples. For
  example, reviewers shown JSON demanded operation identifiers, and a
  generator shown the answer copied its wording. Read samples after every
  prompt change.
- **A shared gateway overloads.** Eight parallel augmentation processes
  returned HTTP 503 and restarted a backing model server. Use one process
  with 2-4 workers.
- **Some answers cannot be right in the measurement world.** Fixture
  grounding cannot render `power_set` balanced or low-power, so at most 29
  of 32 validation proposals (31 of 32 on test) can be right.

## Not verified

- Routes other than Python unsloth: `unsloth-cli`, remote Hugging Face Jobs,
...
Filed as each step happens; the guide above was rewritten from it (plan task
t17).
