# Tier 2 measurement, 2026-09-25: final-scorer-r3b.q4_k_m-missing-candidate

- Command: `scripts/lfm-finetune/measure.py --split $HOME/lfm-train/work/q53-run/splits/test.json --final --model scorer-r3b.q4_k_m --revision sha256:52061cec322242c4 --label final-scorer-r3b.q4_k_m-missing-candidate --config $HOME/lfm-train/work/q53-run/measure/final-scorer-r3b.q4_k_m-missing-candidate.nvsh.toml --ctx 2048 --ground-snapshot $HOME/lfm-train/work/q53-run/ground-snapshot.json --enable-thinking false --max-logprobs 20000 --predictions $HOME/lfm-train/work/q53-run/final/scorer-r3b.q4_k_m --tokenizer $HOME/lfm-train/work/q53-run/runs/scorer-r3b/merged --scorer served --slice missing-candidate`
...
- Tier 2 settings, identical for every run except the model: engine=llama-server, mode=attach, image=native llama-server $HOME/lfm-train/llama.cpp/build/bin/llama-server (0.00.000.563 I srv llama_server: initializing ... version: 0.4.1-dev (build 11132, commit 633733d0a) built with GNU 13.3.0 for Linux aarch64), ctx=2048, tool_call_parser=qwen3_coder
...
- Serving: engine=llama-server, mode=attach, ctx=2048, image=`native llama-server $HOME/lfm-train/llama.cpp/build/bin/llama-server (0.00.000.563 I srv llama_server: initializing ... version: 0.4.1-dev (build 11132, commit 633733d0a) built with GNU 13.3.0 for Linux aarch64)`, tool_call_parser=qwen3_coder
...
- Acceptance run: no
- Final run: yes
- Final runs on the test side, including this one: 20
...
| False-positive tool calls (proposals on explain/escalate entries) | 13 of 83 |
| False-positive tool calls, 95% bootstrap CI | [8.4%, 24.1%] (n=83) |
...
| ECE (10 equal-width bins) | 0.154 |
| ECE, 95% bootstrap CI | [0.089, 0.234] (n=83) |
| Brier (multi-class) | 0.295 |
| Brier, 95% bootstrap CI | [0.169, 0.436] (n=83) |
| ECE / Brier before --calibration | n/a |
| Tokens generated per decision, mean / median | 0.0 / 0.0 |
| Time to first decision, cold / warm median / warm p95 | 167 ms / 124 ms / 165 ms |
| Decision latency, cold / warm median / warm p95 | 167 ms / 124 ms / 165 ms |
...
| escalate_or_explain | 83 | 83 | 0.154 [0.089, 0.234] (n=83) | 0.295 [0.169, 0.436] (n=83) | 83 of 83 [100.0%, 100.0%] (n=83) | 17.0 / 17.0 |
...
| [0.3, 0.4) | 0 | - | - |
| [0.4, 0.5) | 0 | - | - |
| [0.5, 0.6) | 4 | 0.534 | 0.250 |
| [0.6, 0.7) | 1 | 0.694 | 1.000 |
| [0.7, 0.8) | 3 | 0.768 | 1.000 |
| [0.8, 0.9) | 5 | 0.833 | 0.600 |
| [0.9, 1.0) | 70 | 0.992 | 0.857 |
...
|    0   N/A  N/A          147875      G   ...rack-uuid=3190708988185955192        384MiB |
|    0   N/A  N/A         1258534      C   ...ma.cpp/build/bin/llama-server        829MiB |
