# Tier 2 measurement, 2026-09-24: final-a3.q4_k_m

- Command: `scripts/lfm-finetune/measure.py --split $HOME/lfm-train/work/q46/splits/test.json --final --model a3.q4_k_m --revision sha256:214e87f4cc241f2e --label final-a3.q4_k_m --config $HOME/lfm-train/work/q46/measure/final-a3.q4_k_m.nvsh.toml --ctx 2048 --ground-snapshot $HOME/lfm-train/work/q46/ground-snapshot.json --enable-thinking false --max-logprobs 22 --predictions $HOME/lfm-train/work/q46/final/a3.q4_k_m`
...
- Models (repo id @ revision): `a3.q4_k_m` @ `sha256:214e87f4cc241f2e` (operator-supplied, not verified: attached endpoint)
- Tier 2 settings, identical for every run except the model: engine=llama-server, mode=attach, image=native llama-server /home/spark/lfm-train/llama.cpp/build/bin/llama-server (0.00.000.392 I srv llama_server: initializing ... version: 0.4.1-dev (build 11132, commit 633733d0a) built with GNU 13.3.0 for Linux aarch64), ctx=2048, tool_call_parser=qwen3_coder
- Grounding: fixed snapshot `$HOME/lfm-train/work/q46/ground-snapshot.json` (platform: fixture world from the split file)
- Serving: engine=llama-server, mode=attach, ctx=2048, image=`native llama-server /home/spark/lfm-train/llama.cpp/build/bin/llama-server (0.00.000.392 I srv llama_server: initializing ... version: 0.4.1-dev (build 11132, commit 633733d0a) built with GNU 13.3.0 for Linux aarch64)`, tool_call_parser=qwen3_coder
...
- Acceptance run: no
- Final run: yes
- Final runs on the test side, including this one: 14
...
| Metric | `a3.q4_k_m` |
|---|---|
| Model revision | `sha256:214e87f4cc241f2e` (operator-supplied, not verified: attached endpoint) |
...
| Warm latency, median / p95 | 245 ms / 399 ms |
...
| Tokens generated per decision, mean / median | 45.9 / 40.0 |
| Time to first decision, cold / warm median / warm p95 | 353 ms / 244 ms / 398 ms |
| Decision latency, cold / warm median / warm p95 | 353 ms / 244 ms / 399 ms |
| Non-empty think blocks (must be 0) | 0 |

- `a3.q4_k_m`: lines without a candidate distribution: an alternative token's continuation was not observed: 64
...
93050d5259c1   climate-weather-tracker          "python -m climate.w…"   26 minutes ago   Up 26 minutes                                                                                                              weather-tracker
21571f98493d   climate-weather-web              "python -m climate.w…"   26 minutes ago   Up 26 minutes         127.0.0.1:8095->8095/tcp                                                                             weather-web
...
|    0   N/A  N/A         3238092      C   ...ma.cpp/build/bin/llama-server        829MiB |
