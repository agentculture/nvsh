# Tier 2 measurement, 2026-09-26: final-a3-heal.q4_k_m

- Command: `scripts/lfm-finetune/measure.py --split $HOME/lfm-train/work/i64-a3heal/splits/test.json --final --model a3-heal.q4_k_m --revision sha256:8dfd6ddc788c4d70 --label final-a3-heal.q4_k_m --config $HOME/lfm-train/work/i64-a3heal/measure/final-a3-heal.q4_k_m.nvsh.toml --ctx 2048 --ground-snapshot $HOME/lfm-train/work/q53-run/ground-snapshot.json --enable-thinking false --max-logprobs 22 --predictions $HOME/lfm-train/work/i64-a3heal/final/a3-heal.q4_k_m`
...
- Acceptance run: no
- Final run: yes
- Final runs on the test side, including this one: 25
...
| Right operation and arguments proposed, per source | 64 of 83 |
| Right operation and arguments proposed, per variation | 64 of 83 |
| Should-escalate asks escalated, per source | 72 of 79 |
| Should-escalate asks escalated, per variation | 72 of 79 |
| Wrong mutating proposals, per source / per variation | 1 / 1 |
| Mutating proposals with wrong arguments, per source / per variation | 5 / 5 |
| Explain asks explained, per source | 36 of 36 |
| Explain asks: explained / proposed / escalated, per variation | 36 / 0 / 0 of 36 |
| Mutating proposals on explain asks | 0 |
| Warm latency, median / p95 | 225 ms / 353 ms |
...
| Metric | `a3-heal.q4_k_m` |
|---|---|
| Right proposals (metrics.py) | 77 of 83 |
| Right proposals, 95% bootstrap CI | [86.7%, 97.6%] (n=83) |
| Abstention recall (escalate entries escalated) | 89.9% (71 of 79) |
| Abstention recall, 95% bootstrap CI | [83.5%, 96.2%] (n=79) |
| Abstention precision, strict (deviation d2) | 100.0% |
| Abstention precision, strict, 95% bootstrap CI | [100.0%, 100.0%] (n=198) |
| False-positive tool calls (proposals on explain/escalate entries) | 8 of 115 |
| False-positive tool calls, 95% bootstrap CI | [2.6%, 11.3%] (n=115) |
| Wrong mutating, total (wrong operation + wrong arguments) | 4 (2 + 2) |
| Invalid outputs | 0 of 198 |
| Invalid outputs, 95% bootstrap CI | [0.0%, 0.0%] (n=198) |
...
| Tokens generated per decision, mean / median | 41.4 / 40.0 |
| Time to first decision, cold / warm median / warm p95 | 308 ms / 224 ms / 348 ms |
| Decision latency, cold / warm median / warm p95 | 309 ms / 225 ms / 353 ms |
| Non-empty think blocks (must be 0) | 0 |

- `a3-heal.q4_k_m`: lines without a candidate distribution: an alternative token's continuation was not observed: 198
...
|    0   N/A  N/A          147875      G   ...rack-uuid=3190708988185955192        366MiB |
|    0   N/A  N/A         3248148      C   ...ma.cpp/build/bin/llama-server        829MiB |
