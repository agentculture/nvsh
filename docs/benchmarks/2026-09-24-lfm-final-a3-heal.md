...
- Command: `scripts/lfm-finetune/measure.py --split $HOME/lfm-train/work/q46/splits/test.json --final --model a3-heal --revision ef2020edd8981ac816cbfaae249de2c7b806a77d --label final-a3-heal --config $HOME/lfm-train/work/q46/measure/final-a3-heal.nvsh.toml --ctx 2048 --ground-snapshot $HOME/lfm-train/work/q46/ground-snapshot.json --enable-thinking false --max-logprobs 22 --predictions $HOME/lfm-train/work/q46/final/a3-heal`
...
- Models (repo id @ revision): `a3-heal` @ `ef2020edd8981ac816cbfaae249de2c7b806a77d` (operator-supplied, not verified: attached endpoint)
...
- Acceptance run: no
- Final run: yes
- Final runs on the test side, including this one: 16
...
| Metric | `a3-heal` |
|---|---|
| Model revision | `ef2020edd8981ac816cbfaae249de2c7b806a77d` (operator-supplied, not verified: attached endpoint) |
...
| Explain asks explained, per source | 16 of 17 |
| Explain asks: explained / proposed / escalated, per variation | 16 / 1 / 0 of 17 |
| Mutating proposals on explain asks | 0 |
| Warm latency, median / p95 | 430 ms / 687 ms |
...
| Tokens generated per decision, mean / median | 45.3 / 40.0 |
| Time to first decision, cold / warm median / warm p95 | 17244 ms / 429 ms / 686 ms |
| Decision latency, cold / warm median / warm p95 | 17244 ms / 430 ms / 686 ms |
| Non-empty think blocks (must be 0) | 0 |

- `a3-heal`: lines without a candidate distribution: an alternative token's continuation was not observed: 64
...
8fa21e0d4103   vllm/vllm-openai                 "vllm serve --model …"   2 minutes ago    Up 2 minutes          127.0.0.1:18060->8000/tcp                                                                            q46-measure-18060
93050d5259c1   climate-weather-tracker          "python -m climate.w…"   41 minutes ago   Up 41 minutes                                                                                                              weather-tracker
21571f98493d   climate-weather-web              "python -m climate.w…"   41 minutes ago   Up 41 minutes         127.0.0.1:8095->8095/tcp                                                                             weather-web
...
|    0   N/A  N/A          147875      G   ...rack-uuid=3190708988185955192        187MiB |
|    0   N/A  N/A         3278976      C   VLLM::EngineCore                       8734MiB |
