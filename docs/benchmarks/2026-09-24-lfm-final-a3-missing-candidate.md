# Tier 2 measurement, 2026-09-24: final-a3-missing-candidate

- Command: `scripts/lfm-finetune/measure.py --split $HOME/lfm-train/work/q46/splits/test.json --final --model a3 --revision d0303706be884c81e3e72d32042be96d621e160d --label final-a3-missing-candidate --config $HOME/lfm-train/work/q46/measure/final-a3-missing-candidate.nvsh.toml --ctx 2048 --ground-snapshot $HOME/lfm-train/work/q46/ground-snapshot.json --enable-thinking false --max-logprobs 22 --predictions $HOME/lfm-train/work/q46/final/a3 --slice missing-candidate`
...
- Acceptance run: no
- Final run: yes
- Final runs on the test side, including this one: 6
...
| Should-escalate asks escalated, per source | 8 of 32 |
| Should-escalate asks escalated, per variation | 8 of 32 |
...
| Warm latency, median / p95 | 437 ms / 1615 ms |
...
| Abstention recall (escalate entries escalated) | 15.6% (5 of 32) |
| Abstention precision, strict (deviation d2) | 100.0% |
| False-positive tool calls (proposals on explain/escalate entries) | 24 of 32 |
| Wrong mutating, total (wrong operation + wrong arguments) | 5 (5 + 0) |
| Invalid outputs | 2 of 32 (no_decision: 2) |
| Lines with a candidate distribution | 0 of 32 |
| ECE (10 equal-width bins) | n/a |
| Brier (multi-class) | n/a |
| Tokens generated per decision, mean / median | 55.5 / 40.0 |
| Time to first decision, cold / warm median / warm p95 | 17156 ms / 426 ms / 515 ms |
| Decision latency, cold / warm median / warm p95 | 17157 ms / 437 ms / 1614 ms |
| Non-empty think blocks (must be 0) | 0 |

- `a3`: lines without a candidate distribution: an alternative token's continuation was not observed: 30; invalid output: 2
...
2e18842cb84b   vllm/vllm-openai                 "vllm serve --model …"   2 minutes ago    Up 2 minutes          127.0.0.1:18060->8000/tcp                                                                            q46-measure-18060
0205ea8dabbf   climate-weather-web              "python -m climate.w…"   44 minutes ago   Up 44 minutes         127.0.0.1:8095->8095/tcp                                                                             weather-web
93b52a3f60bb   climate-weather-tracker          "python -m climate.w…"   44 minutes ago   Up 44 minutes                                                                                                              weather-tracker
...
|    0   N/A  N/A          147875      G   ...rack-uuid=3190708988185955192        183MiB |
|    0   N/A  N/A         2674366      C   VLLM::EngineCore                       8494MiB |
