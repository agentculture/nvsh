# Tier 2 measurement, 2026-09-24: final-a3-heal.awq

- Command: `scripts/lfm-finetune/measure.py --split $HOME/lfm-train/work/q46/splits/test.json --final --model a3-heal.awq --revision sha256:8dfd6ddc788c4d70 --label final-a3-heal.awq --config $HOME/lfm-train/work/q46/measure/final-a3-heal.awq.nvsh.toml --ctx 2048 --ground-snapshot $HOME/lfm-train/work/q46/ground-snapshot.json --enable-thinking false --max-logprobs 22 --predictions $HOME/lfm-train/work/q46/final/a3-heal.awq`
...
- Models (repo id @ revision): `a3-heal.awq` @ `sha256:8dfd6ddc788c4d70` (operator-supplied, not verified: attached endpoint)
...
- Acceptance run: no
- Final run: yes
- Final runs on the test side, including this one: 17
...
| Right operation and arguments proposed, per source | 29 of 32 |
| Right operation and arguments proposed, per variation | 29 of 32 |
...
| Warm latency, median / p95 | 296 ms / 528 ms |
| First request after start | 18.4 s |
...
| Metric | `a3-heal.awq` |
|---|---|
| Right proposals (metrics.py) | 30 of 32 |
...
| Invalid outputs | 1 of 64 (no_decision: 1) |
| Lines with a candidate distribution | 0 of 64 |
| ECE (10 equal-width bins) | n/a |
| Brier (multi-class) | n/a |
| Tokens generated per decision, mean / median | 47.6 / 41.5 |
| Time to first decision, cold / warm median / warm p95 | 18375 ms / 294 ms / 513 ms |
| Decision latency, cold / warm median / warm p95 | 18376 ms / 296 ms / 528 ms |
| Non-empty think blocks (must be 0) | 0 |

- `a3-heal.awq`: lines without a candidate distribution: an alternative token's continuation was not observed: 63; invalid output: 1
...
bb6922911a08   vllm/vllm-openai                 "vllm serve --model …"   2 minutes ago    Up 2 minutes          127.0.0.1:18060->8000/tcp                                                                            q46-measure-18060
...
|    0   N/A  N/A         3287908      C   VLLM::EngineCore                       8840MiB |
