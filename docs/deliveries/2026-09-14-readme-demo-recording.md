...
- PR #14 review (Qodo, ten inline findings, all FIX): fixed in `b627bb1` and recorded as delta `b6` with evidence `e18`–`e20`. The demo's script token is command-position only and shell-safe (a `./model.sh;id` token can no longer ride into the approved `chmod`), fixtures are shape-checked and `[agents.demo] fixture` is a valid config key, setup refuses demo through an alias or `@`/model form and never keeps a stored demo default, the recorder scrubs across pty read boundaries and `--clean-env` drops `NVSH_*`/`XDG_*`, and the driver pins `XDG_CACHE_HOME`/`NVSH_NO_DAEMON` and refuses to write an incomplete cast. Six SonarCloud code smells fixed in `11b4c92`/`9ef269f` (the first of those was pushed ungated: lapse `l6`). SonarCloud then raised S6350 (argument injection through `shlex.quote`): fixed in `3eb21ee` by allowlisting both the token and the fixture default, refusing a leading `-`, dropping the quote call and ending option parsing with `chmod +x -- <script>`; the three devices were re-recorded with that command in `9ad7ee9` (delta `b7`, evidence `e21`).
...
- tests: `uv run pytest -n auto -q` at `13b2edf` — 1836 passed, 5 skipped; at `b627bb1` (after review fixes) — 1858 passed; at `9ad7ee9` (after the S6350 fix and re-record) — 1859 passed, 5 skipped (pre-existing live-harness skips)
...
- commits: `ff52eda..9ad7ee9` (33 commits on `feat/readme-demo-recording`)
...
| the recording is regenerated by one committed driver in a sandbox that never touches the operator's rc file, config or daemon (exercised three times per device during this run) | high | file `scripts/demo-record.py` · test `tests/test_demo_record.py::test_demo_record_end_to_end` · evidence `e7` |
