...
use-case bar, the headline of the announcement, is not met, and review found
four test entries in r8's training (lapse `l5`), so the final figures are an
upper bound.
...
| `t17` | delivered | `docs/lfm-finetune.md` rewritten as the verified guide (`7cda29a`) with the full run log; the model card is generated by `release_bundle.py` and live on the private repo; the dataset card and manifest by `dataset_bundle.py` (added after review) |
...
| `t17` | Dataset card delivered late, as a generator (`dataset_bundle.py`) rather than a static draft | `acceptable` |
| `t14` | Iteration on validation only, except that one grep printed one test-side entry (lapse `l2`) | `risky` |
| `t13`/`t16` | Four test entries (and one validation entry) reached r8's training by exact wording: supplement `sup-07` and three generated variations; final run 2 is an upper bound (lapse `l5`). Fixed for future runs by `merge_variations.py --exclude` and a supplement test | `needs-follow-up` |
...
|-------|------------|----------|
| Tuned r8 beats stock on the test side by a wide margin: as measured 28/32 vs 0/32 right proposals, 13/15 vs 0/15 escalations, 17/17 vs 15/17 explained, 119 vs 290 ms; without the four leaked test entries at least 27/31, 12/14, 15/15 | medium | `docs/benchmarks/2026-09-23-lfm-final-r8.md` · `e1`, `e2`; capped by pending lapse `l5` (4 test entries in training) |
...
| A dataset card and provenance manifest can be built | high | `scripts/lfm-finetune/dataset_bundle.py` · `tests/test_lfm_finetune_dataset_bundle.py`; not uploaded |
...
- A clean final measurement: with the leakage fix, re-assemble, retrain and measure on a test side that no training record repeats (lapse `l5`). The next final run should use a re-seeded or new test side, since this one has now been read in part (`l2`, `l5`).
...
- `t17` dataset card: drafted afterwards as `scripts/lfm-finetune/dataset_bundle.py`
  (card, per-record provenance manifest, splits). Build it after a
  re-assemble with `--exclude`; it refuses leaked records. Not uploaded.
